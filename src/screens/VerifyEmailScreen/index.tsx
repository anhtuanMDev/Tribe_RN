...
const VerifyEmailScreen = ({
  ...
  route: RouteProp<FlowCredential, typeof ROUTES.VERIFY_EMAIL>;
}) => {
  ...
  const { mutate, status } = useResendCode();

  const resendCode = () => {
    mutate({ email: state.email ?? '', purpose });
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <KeyboardAwareView>
        ...
        <Typography style={styles.contextText} level="labelMedium">
          Didn't receive a code?
        </Typography>

        <ResendCode onResend={resendCode} status={status} />
      </KeyboardAwareView>
    </SafeAreaView>
  );
};
